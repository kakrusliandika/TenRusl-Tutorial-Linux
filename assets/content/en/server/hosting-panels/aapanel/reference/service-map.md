# 📋 Service Map
    ...
    ## Purpose
    Keep one operational view of the services aaPanel depends on so control-plane changes are always cross-checked against real host state.
...
| Runtime layer | Runtime work in aaPanel often means PHP versions, site-level rewrites, cron jobs, SSL state, and database mappings created from the panel. | Check PHP-FPM, upstream processes, or containers explicitly. |
