# 🌐 aaPanel
    ...
    ## Overview
    This module treats aaPanel as a Linux hosting control plane, not as magic. The focus is host preparation, site onboarding, runtime verification, admin-surface control, backup scope, and shell-based recovery when the panel or generated config does not behave as expected. aaPanel is a hosting control plane that often bundles web, runtime, and database administration into one surface. Treat it as a control plane layered on top of Linux, not as a replacement for Linux operations.

    ## Who This Section Is For
    - Operators running several PHP or mixed web workloads on one VPS or small fleet who still need shell-level recovery.
    ...
    ## Learning Path
    1. Start with [🧠 aaPanel Overview](./concepts/aapanel-overview.md) and the concept pages so the control-plane boundaries are clear.
...
4. Walk the tutorial series on a lab host before trusting the panel with real domains or customer data.
    ...
    ## What To Read First
    - Concept: [🧠 aaPanel Overview](./concepts/aapanel-overview.md)
    ...
    ## Section Map
### Concepts
- [🧠 aaPanel Overview](./concepts/aapanel-overview.md)
