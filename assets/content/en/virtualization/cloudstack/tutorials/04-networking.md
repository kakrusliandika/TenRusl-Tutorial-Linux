...
## Objective
Create or validate the first intentional network layout in CloudStack so management and guest traffic stay understandable.
...
```bash
cloudmonkey list networkofferings 2>/dev/null || true
cloudmonkey create network name=lab-guest displaytext="Lab Guest" networkofferingid=<NETWORK_OFFERING_ID> zoneid=<ZONE_ID> 2>/dev/null || true
cloudmonkey list networks keyword=lab-guest 2>/dev/null || true
```
