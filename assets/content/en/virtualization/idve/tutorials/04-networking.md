...
## Objective
Create or validate the first intentional network layout in IDVE so management and guest traffic stay understandable.
...
```bash
sudo nmcli connection add type bridge ifname br-tenant con-name br-tenant
sudo nmcli connection add type bridge-slave ifname eth1 master br-tenant
nmcli connection show br-tenant
```
