...
## Tujuan
Melatih restore pada IDVE secara sengaja sehingga backup bukan hanya arsip, tetapi benar-benar jalur recovery.
...
```bash
ls -lh /var/backups 2>/dev/null || true
virsh list --all 2>/dev/null || true
journalctl -xe | grep -Ei "idve|libvirt|qemu|virt" | tail -n 40 || true
```
