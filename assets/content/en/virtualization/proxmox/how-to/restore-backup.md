...
## Objective
Restore a backup or export into a safe target on Proxmox and prove the recovered workload is operational, not just present.
...
```bash
qmrestore /srv/pve-backup/vzdump-qemu-100.vma.zst 200 --storage local-lvm --unique 1
qm start 200
qm list
```
