...
## Objective
Restore a backup or export into a safe target on Harvester and prove the recovered workload is operational, not just present.
...
```bash
kubectl get vm -A 2>/dev/null || true
kubectl -n longhorn-system get backups.longhorn.io 2>/dev/null || true
kubectl -n longhorn-system get backupvolumes.longhorn.io 2>/dev/null || true
```
