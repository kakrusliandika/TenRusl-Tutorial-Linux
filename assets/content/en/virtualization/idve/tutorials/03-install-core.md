...
## Objective
Bring the core IDVE control plane into a known-good state before networks, storage, and guest artifacts are expanded.
...
Because IDVE deployments can differ, use the supported product installation flow for your environment, then move quickly into Linux-side validation: CPU virtualization flags, service state, networking, storage layout, and a test guest or image path.
